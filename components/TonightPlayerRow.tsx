...
function injuryLabel(signal: InjurySignal, note: string | null): string {
...
export default function TonightPlayerRow({
  ...
  const context = [matchup, time].filter(Boolean).join(' · ');
    ...
    <View style={styles.wrapper} testID={testID}>
      ...
      <View style={styles.body}>
        ...
        <Text style={styles.reason} numberOfLines={2}>
          {status.reason}
          {status.injurySignal !== 'ok' ? ` · ${injuryLabel(status.injurySignal, status.injuryNote)}` : ''}
        </Text>
      </View>
    </View>
  );
